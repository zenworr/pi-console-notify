...
Route `console.warn()` and `console.error()` through Pi's notification UI. This prevents raw console output from overwriting the terminal input area.
...
- Notifications appear in Pi's conversation area, not as floating toasts.
...
Direct writes to stdout or stderr, subprocess output, and `console.log()` are unchanged. The extension changes how warnings are displayed; it does not fix their causes.
